            ...
            return (
              <div key={module.id} className="card hover:shadow-lg transition-shadow">
                ...
                <div className="flex flex-wrap gap-2 text-xs text-gray-500 mb-4">
                  {module.institution && <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded">📚 {module.institution.name}</span>}
                  {module.course && <span className="px-2 py-1 bg-purple-100 text-purple-800 rounded">📖 {module.course.name}</span>}
                  {module.school && <span className="px-2 py-1 bg-green-100 text-green-800 rounded">🏫 {module.school.name}</span>}
                </div>
                <div className="flex justify-between items-center text-sm text-gray-500 mb-4">
                  <span>{module.duration ? `⏱️ ${module.duration} min` : '⏱️ N/A'}</span>
                  <span>👥 {module._count?.progress || 0} learners</span>
                </div>
